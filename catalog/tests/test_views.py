...
class CreateAuthorViewTest(TestCase):
    def setUp(self):
        #Create a user
        test_user1 = User.objects.create_user(username='testuser1', password='12345')
        permission1 = Permission.objects.get(name='Update author')
        #test_user1.user_permissions.add(permission1)
        test_user1.save()

        test_user2 = User.objects.create_user(username='testuser2', password='12345')
        test_user2.save()
        permission2 = Permission.objects.get(name='Create author')
    ...
    def test_field_with_initial(self):
        ...
        resp = self.client.get(reverse('author_create') )
        #self.assertEqual( resp.status_code,200)
        #tt = resp.render().content
        #print(tt)
        #self.assertContains(tt,'name="date_of_death" value="05/01/2018"')
    ...
    def test_form_first_name_required(self):
        ...
        resp = self.client.get(reverse('author_create') )
        self.assertEqual( resp.status_code,200)
        resp = self.client.post(reverse('author_create'), {'first_name':'','last_name':''} )
        self.assertEqual( resp.status_code,200)
        self.assertFormError(resp, 'form', 'first_name', 'This field is required.')

    def test_form_last_name_required(self):
        login = self.client.login(username='testuser2', password='12345')
        resp = self.client.get(reverse('author_create') )
        self.assertEqual( resp.status_code,200)
        resp = self.client.post(reverse('author_create'), {'first_name':'John','last_name':''} )
        self.assertEqual( resp.status_code,200)
        self.assertFormError(resp, 'form', 'last_name', 'This field is required.')

    def test_form_fill_fields(self):
        login = self.client.login(username='testuser2', password='12345')
        resp = self.client.get(reverse('author_create') )
        self.assertEqual( resp.status_code,200)
        resp = self.client.post(reverse('author_create'), {'first_name':'John','last_name':'Smith'} )
        self.assertEqual( resp.status_code,302)
        #print(resp,reverse('authors'))
